...
33. 결과 엑셀에 '쿼터_실적표' 시트 추가 (utils.build_quota_report)
    - 설정한 쿼터표와 같은 배치로 목표 / 달성 / 차이 세 표를 만든다.
      교차 방향도 쿼터 설정과 같게 잡는다 (마지막 변수를 열, 나머지를 행).
    - 추가 쿼터는 그룹별 빈도표(목표·달성·차이·달성률·구성비).
      조합형은 '달성 / 목표' 를 한 칸에 넣은 교차표로 만든다.
      복수응답 그룹은 구성비 합이 100%를 넘을 수 있다고 제목에 적는다.
    - 통과자 기준이다. SPSS 에서 프리퀀시·크로스탭을 다시 돌리지 않아도
      소프트 쿼터로 어긋난 셀을 바로 확인할 수 있다.
33-D. 쿼터_실적표를 가로 배치로
    - 목표 / 달성 / 차이를 세로로 쌓으니 추가 쿼터가 여러 개일 때 시트가
      한없이 길어졌다. 나란히 놓아 블록 13개 -> 5개, 233행 -> 81행이 됐다.
    - 각 표의 왼쪽 위 모서리 칸에 목표/달성/차이 라벨을 넣는다.
    - [버그] 음수 색칠이 통째로 빠져 있었다. isinstance(v, int) 로 판정했는데
      DataFrame 에서 꺼낸 값은 numpy.int64 라 파이썬 int 가 아니다.
      float(v) 로 바꿔 판정한다. 실제 결과 파일에서 음수 18칸 전부 확인.
33-C. 쿼터_실적표 서식 적용
