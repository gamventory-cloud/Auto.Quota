...
if _version_tuple(sl.__version__) < REQUIRED_SL_VERSION:
    st.error(
        ...
        "구버전은 일부 접두사 문항을 인식하지 못하거나 복수응답 라벨 방식을 지원하지 않습니다.\n"
...
def parse_cached(docx_bytes, base0_key, full_labels, multi_style, module_version):
...
with st.sidebar:
    ...
    MULTI_CHOICES = {
        "열마다 보기 코드 (미선택은 공백)": "category",
        "보기별 열, 자기 코드만": "position",
        "0/1 더미": "dichotomy",
    }
    multi_label = st.radio(
        "복수응답 저장 방식",
        list(MULTI_CHOICES),
        index=0,
        help="열마다 보기 코드: 선택한 보기의 코드가 들어가고 미선택은 공백인 방식. "
