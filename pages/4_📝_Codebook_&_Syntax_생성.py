...
st.title("📝 설문지 읽기 & 코드북/신텍스 자동 생성 (Final Fixed)")
...
# 동그라미 숫자를 아라비아 숫자로 매핑하는 딕셔너리 추가
...
# [FIX] 수평형 척도 표 처리 (B1-1 등) - 숫자 행 감지 강화 & 라벨 매핑 개선
def extract_horizontal_scale_table(table, current_var):
    ...
    # 동그라미 숫자를 일반 숫자로 변환하여 추출
    codes = []
    for cell in rows[numeric_row_idx].cells:
        t = cell.text.strip()
        if not t: continue
...
# (기존 to_excel_with_usage_flag, generate_spss_final, Streamlit UI 로직 동일)
