...
def is_multiple_choice(entry):
    ...
    if re.search(r"\d+[\)\.]", vals) or "=" in vals:
        return True
    if "선택]" in q_text:
        return True
    return False
...
def check_and_split_date(entry):
    ...
    if "억" in val:
        return [entry]
    if re.search(r"(몇\s*명|명\s*수|인원|\(\s*\)\s*명|\[\s*\]\s*명)", val):
        return [entry]
    ...
    has_year = has_unit(val, "년")
    has_month = has_unit(val, "월") or has_unit(val, "개월")
    has_day = has_unit(val, "일")
...
def check_and_split_money(entry):
    ...
    if "만원" not in val_clean and "만 원" not in val:
        return [entry]
...
def extract_double_scale_table(table, current_var):
    ...
    raw_cat_cells = [c.text.strip() for c in rows[0].cells]
    non_empty_cats = [c for c in raw_cat_cells if c]
    if len(non_empty_cats) < 2: return None 
    categories = collapse_consecutive_duplicates(non_empty_cats)
    if len(categories) != 2:
        return None
    
    scale_row_cells = [c.text.strip() for c in rows[1].cells]
    scales = scale_row_cells[1:]
    
    if len(scales) % 2 != 0:
        return None
    
    mid = len(scales) // 2
    left_scale = scales[:mid]
    right_scale = scales[mid:]
    
    left_norm = "".join(left_scale).replace(" ", "")
    right_norm = "".join(right_scale).replace(" ", "")
    ...
    for idx, txt in enumerate(left_scale):
        if txt:
            scale_pairs.append(f"{idx+1}={txt}")
    scale_str = "\n".join(scale_pairs)
    
    cat1_label = categories[0]
    cat2_label = categories[1]
    ...
    for r_idx, row in enumerate(rows[2:]):
        ...
        entry1 = {
            "변수명": f"{var_base}_1",
            "질문 내용": f"[{cat1_label}] {q_text_clean}",
            "보기 값": scale_str,
            "유형": "Scale"
        }
        ...
        entry2 = {
            "변수명": f"{var_base}_2",
            "질문 내용": f"[{cat2_label}] {q_text_clean}",
            "보기 값": scale_str,
            "유형": "Scale"
        }
        
        extracted_entries.append(entry1)
        extracted_entries.append(entry2)
...
def extract_table_scale(table):
    ...
    for cell_text in first_data_row:
        if "입력" in cell_text or "범위" in cell_text or "%" in cell_text:
            numeric_cells.append(None)
            continue
        match = re.search(r"(\d+)", cell_text)
        if match:
            numeric_cells.append(match.group(1))
        else:
            numeric_cells.append(None)
    ...
    if len(first_data_row) > 0 and (body_numeric_count / len(first_data_row)) >= 0.3:
        ...
        for i in range(len(headers)):
            ...
            h_text = headers[i]
            d_val = numeric_cells[i]
            if d_val is not None and h_text:
                scale_pairs.append(f"{d_val}={h_text}")
        if scale_pairs:
...
def is_input_table(table):
    ...
    for row in table.rows:
        if len(row.cells) > 1:
            ...
            if "입력" in cell_text or "(" in cell_text or "%" in cell_text or "_" in cell_text:
                target_count += 1
    if total_rows > 0 and (target_count / total_rows) >= 0.3:
        return True
    return False
...
def extract_multi_column_input_table(table, current_var, force_row_count=None):
    ...
    if len(first_data_row_cells) > 0 and (digit_count / len(first_data_row_cells)) > 0.5:
        return None
    ...
    if force_row_count and force_row_count > actual_data_rows:
        target_loop_count = force_row_count
...
def check_and_split_max_n_text(entry):
    if entry["유형"] != "Single" and entry["유형"] != "Open":
        return None
        
    q_text = entry["질문 내용"]
    if "보기_list" in entry:
        q_text += " " + " ".join(entry["보기_list"])
    ...
    if count == 0 and "3" in q_text_norm and ("기입" in q_text_norm or "작성" in q_text_norm):
         count = 3
         
    if count < 1:
        return None
        
    has_manufacturer = "제조사" in q_text_norm
    has_brand = "브랜드" in q_text_norm
    
    new_entries = []
    for i in range(1, count + 1):
        if has_manufacturer and has_brand:
            v1 = entry.copy()
            v1["변수명"] = f"{entry['변수명']}_{i}_1"
            v1["질문 내용"] = f"[{entry['변수명']}] {i}순위 - 제조사"
            v1["유형"] = "Open"
            if "보기_list" in v1: del v1["보기_list"]
            
            v2 = entry.copy()
            v2["변수명"] = f"{entry['변수명']}_{i}_2"
            v2["질문 내용"] = f"[{entry['변수명']}] {i}순위 - 브랜드"
            v2["유형"] = "Open"
            if "보기_list" in v2: del v2["보기_list"]
            
            new_entries.append(v1)
...
def parse_word_to_df(docx_file):
    ...
    def flush_entry(entry):
        ...
        if pending_max_n_count is not None:
            has_manufacturer = "제조사" in entry["질문 내용"]
            has_brand = "브랜드" in entry["질문 내용"]
            new_entries = []
        ...
        if "D6_2" in entry["변수명"].replace("-", "_"):
            is_multi = True
    ...
    for block in iter_block_items(doc):
        if isinstance(block, Paragraph):
            ...
            if re.match(r"^\[PROG", text, re.IGNORECASE) or re.match(r"^\(PROG", text, re.IGNORECASE):
                continue
            ...
            if is_new_q:
                ...
                if inline_opts:
                    first_opt = inline_opts[0]
                    split_idx = label.find(first_opt)
                    if split_idx != -1:
                        q_text = label[:split_idx].strip()
                ...
                if rank_count:
                    pending_ranking_count = rank_count
                    ranking_options_buffer = [] 
                else:
                    pending_ranking_count = None
                    ranking_options_buffer = []
                ...
                if "1개 선택" in current_entry["질문 내용"]:
                    current_entry["유형"] = "Single"
                ...
                if not is_parent_added:
                    ...
                    if opts_in_line:
                        ...
                        if "보기_list" in current_entry and not current_entry["보기_list"]:
            ...
            if current_entry and not is_parent_added:
                ...
                if q_type in ["Single", "Multi"]:
                    ...
                    if re.match(r"^(\d+|[①-⑩]|[a-zA-Z])[\)\.]", first_cell_text):
                        is_opt_table = True
            ...
            if current_entry:
                ...
                if multi_col_entries:
                    extracted_data.extend(multi_col_entries)
                    is_parent_added = True
                    pending_max_n_count = None
                    continue
            ...
            if is_input_style:
                ...
                if len(rows) > 1:
                    for row in rows[1:]:
                        ...
                        if fc and not fc.isdigit() and fc not in ["○", "●", "V"]: 
                ...
                if not is_parent_added:
                    ...
                    if not is_matrix_table and not is_input_style:
                        ...
                        if len(split_entries) == 1: split_entries = check_and_split_money(split_entries[0])
                        if len(split_entries) == 1: split_entries = check_and_split_percent(split_entries[0])
                        extracted_data.extend(split_entries)
                        is_parent_added = True
                    else:
                ...
                if is_matrix_table:
                    ...
                    for row in rows[1:]:
                        ...
                        first_cell = row.cells[0].text.strip()
                        if not first_cell: continue
                        match_matrix_var = var_pattern.match(first_cell)
                        m_var = ""; m_label = ""; m_vals = table_vals_str if table_vals_str else "(헤더참조)"
                        if match_matrix_var:
                            pot_var = match_matrix_var.group(1)
                            if re.match(r"^[A-Z][A-Z0-9\-_]*$", pot_var) or re.match(r"^\d+[\-\_]\d+$", pot_var):
                                m_var = pot_var.replace("-", "_")
                                m_label = match_matrix_var.group(2)
                            else:
                                sub_item_count += 1
                                parent_var = current_entry["변수명"]
                                m_var = f"{parent_var}_{sub_item_count}"
                                m_label = f"[{parent_var} 세부] {first_cell}"
                        else:
...
def to_excel_with_usage_flag(df):
    ...
    for idx, row in df.iterrows():
        ...
        if "_" in var_name:
            ...
            final_q_label = f"{var_name}. {clean_q}"
...
def compress_var_list(var_list):
    ...
    for var in var_list:
        ...
        prev_var = current_chunk[-1]
        match_prev = pattern.match(prev_var); match_curr = pattern.match(var)
        is_continuous = False
        if match_prev and match_curr:
            prev_prefix, prev_num = match_prev.groups()
            curr_prefix, curr_num = match_curr.groups()
...
def generate_spss_final(df_edited, encoding_type='utf-8'):
    ...
    for idx, row in df_target.iterrows():
        if row['사용여부'] == 'R':
            ...
            if codes:
                min_c, max_c = min(codes), max(codes)
                recode_pairs = []; 
                for c in codes: 
                    new_c = max_c + min_c - c; 
                    recode_pairs.append(f"({c}={new_c})")
    ...
    for idx, row in df_target.iterrows():
        ...
        for line in lines:
            line = line.strip()
            if '=' in line:
                parts = line.split('=', 1); code = parts[0].strip(); label = parts[1].strip()
                final_code = code
    ...
    syntax_lines.append(""); syntax_lines.append("* 4. Save Data.")
    
    keep_vars = df_target['변수명'].drop_duplicates().tolist()
    if keep_vars:
        syntax_lines.append("SAVE OUTFILE='Project_DATA.sav'")
        syntax_lines.append("  /KEEP=")
    ...
    syntax_lines.append("EXECUTE.")
    syntax_lines.append(""); syntax_lines.append("* 5. Export to Excel."); syntax_lines.append("GET FILE='Project_DATA.sav'."); syntax_lines.append("EXECUTE.")
