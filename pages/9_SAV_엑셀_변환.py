...
SPSS .sav 파일을 업로드하면 3개 시트로 구성된 엑셀 파일을 내려받습니다.
  · Raw        : 숫자 코드 그대로
...
try:
    df, col_labels, value_labels = read_sav_bytes(up.getvalue(), up.name)
...
want_guide = s3.checkbox("변수 가이드", value=True)

if not (want_raw or want_label or want_guide):
