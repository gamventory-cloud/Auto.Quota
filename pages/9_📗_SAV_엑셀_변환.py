...
ID 로 짝을 맞춰 SAV 값을 덮어쓰고 새 SAV 를 내려받습니다.
...
def apply_patch(df: pd.DataFrame, value_labels: dict, var_types: dict,
                patch: pd.DataFrame, sav_key: str, patch_key: str) -> tuple:
    """엑셀 값을 SAV 데이터에 덮어쓴다.

    · ID 로 짝을 맞추고, 이름이 같은 변수만 덮어쓴다 (대소문자 무시).
    ...
    # ── 변수 짝 맞추기 (대소문자 무시) ──
    sav_by_lower = {str(c).lower(): c for c in out.columns}
    pairs, only_in_patch = [], []
    for pc in patch.columns:
        if str(pc) == patch_key:
            continue
        sc = sav_by_lower.get(str(pc).lower())
        if sc is None or sc == sav_key:
...
tab_export, tab_patch = st.tabs(["엑셀로 풀기", "엑셀 값 반영하기"])
...
with tab_export:
    ...
    if st.button("엑셀로 변환하기", type="primary", use_container_width=True):
        with st.spinner("엑셀 파일을 만드는 중입니다…"):
            try:
                xlsx_bytes = to_excel(sheets, head_color)
            except Exception as e:
...
with tab_patch:
    st.caption(
        "코딩이나 수정을 마친 엑셀을 올리면 ID 로 짝을 맞춰 같은 이름의 변수를 "
    ...
    if sheet_names:
        if len(sheet_names) == 1:
            ...
            # 이름에 '코딩', '수정' 같은 말이 들어간 시트를 먼저 짚어 준다
    ...
    with k1:
        sav_key = st.selectbox(
            ...
            index=list(df.columns).index(key_cols[0]) if key_cols else 0,
        )
    with k2:
    ...
    with st.spinner("맞춰 보는 중입니다…"):
        new_df, new_labels, rep = apply_patch(
            df, value_labels, var_types, patch, sav_key, patch_key)
    ...
    if rep["only_in_patch"]:
        st.info(
            f"SAV 에 없는 열 {len(rep['only_in_patch'])}개는 넘겼습니다 — "
    ...
    if not rep["changed"]:
        st.info("덮어쓸 값이 없습니다. ID 열과 변수 이름을 확인해 주세요.")
        st.stop()
    ...
    if st.button("반영한 SAV 만들기", type="primary", use_container_width=True):
        with st.spinner("SAV 파일을 만드는 중입니다…"):
            try:
                sav_bytes = write_sav(new_df, col_labels, new_labels)
            except Exception as e:
                st.error(f"SAV 생성에 실패했습니다: {e}")
                st.stop()
        st.session_state["sx_sav_bytes"] = sav_bytes
        st.session_state["sx_sav_name"] = (
            os.path.splitext(up.name)[0] + "_반영.sav")

    if st.session_state.get("sx_sav_bytes"):
        st.success("반영이 끝났습니다.")
        st.download_button(
            "SAV 파일 내려받기",
            data=st.session_state["sx_sav_bytes"],
            file_name=st.session_state["sx_sav_name"],
